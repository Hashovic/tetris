...
const tetris = p => {
    ...
    p.setup = () => {
        ...
        */
        ...
        // play area
        p.canvas = p.createCanvas(GRID_WIDTH * B_SIZE * 2, GRID_HEIGHT * B_SIZE);
        p.held_canvas = p.createGraphics(p.floor(GRID_HEIGHT / 4) * B_SIZE, p.floor(GRID_HEIGHT / 4)  * B_SIZE - B_SIZE);
        p.next_canvas = p.createGraphics(p.floor(GRID_HEIGHT / 4) * B_SIZE, GRID_WIDTH * 1.5 * B_SIZE);
